...
bool VoiceInputManager::init() {
    ...
    recognizer_->setAppId(config_.appId.value());
    recognizer_->setAccessToken(config_.accessToken.value());
    recognizer_->setSecretKey(config_.secretKey.value());
    recognizer_->setCluster(config_.cluster.value());
    recognizer_->setAuthType(config_.authType.value());
